...
before the protocol is battle-tested.
...
All non-emergency parameter changes go through a **24–72 hour timelock**:

1. **Proposal**: a multisig signer creates a transaction on the timelock
   contract.
2. **Queue**: the timelock contract stores the call data and an `eta` timestamp.
3. **Execute**: after `eta`, anyone can call `execute()` to apply the change.

During the timelock window, the community can verify the proposed change and
exit if it is malicious.
...
At that point, the multisig will be replaced by token-weighted governance with
delegation (similar to Compound's Governor Bravo).

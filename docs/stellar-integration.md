...
> How OpenLend follows Stellar Ecosystem Proposals (SEPs) and integrates
...
The OpenLend contracts are deployed as Soroban **WASM** contracts on the
...
Contract addresses are published in `sdk/src/manifest.json` and in the
project's `stellar.toml`.

## Event Streaming

OpenLend uses Stellar Horizon's **transaction streaming** to observe
contract events in near-real-time:

1. The `indexer` service subscribes to Horizon's
   `/accounts/{CONTROLLER}/operations` SSE stream.
2. Each `invokeHostFunction` operation is parsed and persisted to
   Postgres.
3. The `api` WebSocket layer pushes filtered events to subscribed
   clients.

## Stellar Asset Information

Wrapped tokens (`wETH`, `wUSDC`, etc.) and deposit tokens (`lETH`,
`lUSDC`, etc.) are issued on Stellar and described in the project's
[`stellar.toml`](../stellar.toml) (SEP-0001). Each asset entry includes
the issuer, asset code, name, description, and display decimals.
